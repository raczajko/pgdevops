...
class PickleSession(MutableMapping, SessionMixin):
    ...
    def read(self):
        ...
        except (OSError, IOError, ValueError, EOFError, UnpicklingError):
            self.data = {}
...
class PickleSessionInterface(SessionInterface):
    ...
    def __init__(self, directory):
        ...
        self.cookie_timedelta = datetime.timedelta(days=1)
    ...
    def open_session(self, app, request):
        pid_digest = hashlib.sha224(str(os.getpid())).hexdigest()
        uid_digest = hashlib.sha224(str(uuid1())).hexdigest()
        sid = request.cookies.get(
            app.session_cookie_name) or '{}!{}'.format(uid_digest, pid_digest)
